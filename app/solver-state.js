...
function getEdgeOrientation(colors, slot) {
    if (!Array.isArray(colors) || colors.length !== 2) {
        return null;
    }

    const actualSorted = [...colors].sort().join("|");
    let pieceName = null;

    for (const [name, pieceColors] of Object.entries(
        RUBIX_SOLVER_STATE.edgeColors
    )) {
        const expectedSorted = [...pieceColors].sort().join("|");
    ...
    const canonical = RUBIX_SOLVER_STATE.edgeColors[pieceName];

    if (
        colors[0] === canonical[0] &&
        colors[1] === canonical[1]
    ) {
        return 0;
    }

    if (
        colors[0] === canonical[1] &&
        colors[1] === canonical[0]
    ) {
        return 1;
    }
